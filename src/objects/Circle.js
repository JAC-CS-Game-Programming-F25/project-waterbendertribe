...
import { CANVAS_HEIGHT, CANVAS_WIDTH, context, matter } from "../globals.js";
import Ground from "./Ground.js";

export default class Circle extends GameMatter {
  ...
  didStop() {
    return (
      this.body.angularSpeed < Circle.ANGULAR_SPEED_MINIMUM &&
      this.body.speed < Circle.SPEED_MINIMUM
    );
  }

  didGoOffScreen() {
    return (
      this.body.position.x + this.radius < 0 ||
      this.body.position.x - this.radius > CANVAS_WIDTH
    );
  }

  isOnGround() {
    return (
      this.body.position.y > CANVAS_HEIGHT - Ground.GRASS.height - this.radius
    );
  }
}
